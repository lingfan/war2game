...
$PHP_SELF = $PHP_SELF . '?r=Server/Apc';
...
function sortheader($MYREQUEST, $key, $name, $extra = '') {
	...
	return "<a class=sortable href=\"$extra&SORT1=$key&SORT2=" . $MYREQUEST['SORT2'] . "\">$name</a>";
...
EOB;
echo
menu_entry(1, 'View Host Stats', $MYREQUEST, $MY_SELF),
menu_entry(2, 'System Cache Entries', $MYREQUEST, $MY_SELF);
if ($AUTHENTICATED) {
	echo menu_entry(4, 'Per-Directory Entries', $MYREQUEST, $MY_SELF);
}
echo
menu_entry(3, 'User Cache Entries', $MYREQUEST, $MY_SELF),
menu_entry(9, 'Version Check', $MYREQUEST, $MY_SELF);
...
EOB;
		...
		$cols = 6;
		echo <<<EOB
		<div class=sorting><form action="$PHP_SELF" method="get">Scope:
		<input type=hidden name=OB value={$MYREQUEST['OB']}>
		<input type=hidden name=r value="Server/Apc">
...
EOB;
		...
		'<div class="info"><table cellspacing=0><tbody>',
		'<tr>',
		'<th>', sortheader($MYREQUEST, 'S', $fieldheading, "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'<th>', sortheader($MYREQUEST, 'H', 'Hits', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'<th>', sortheader($MYREQUEST, 'Z', 'Size', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'<th>', sortheader($MYREQUEST, 'A', 'Last accessed', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'<th>', sortheader($MYREQUEST, 'M', 'Last modified', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'<th>', sortheader($MYREQUEST, 'C', 'Created at', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>';

		if ($fieldname == 'info') {
			$cols += 2;
			echo '<th>', sortheader($MYREQUEST, 'T', 'Timeout', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>';
		}
		echo '<th>', sortheader($MYREQUEST, 'D', 'Deleted at', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th></tr>';
...
// -----------------------------------------------
	case OB_SYS_CACHE_DIR:
		...
		echo <<<EOB
		<div class=sorting><form action=>Scope:
		<input type=hidden name=OB value={$MYREQUEST['OB']}>
		<select name=SCOPE>
EOB;
		echo
		...
		'<div class="info"><table cellspacing=0><tbody>',
		'<tr>',
		'<th>', sortheader($MYREQUEST, 'S', 'Directory Name', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'<th>', sortheader($MYREQUEST, 'T', 'Number of Files', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'<th>', sortheader($MYREQUEST, 'H', 'Total Hits', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'<th>', sortheader($MYREQUEST, 'Z', 'Total Size', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'<th>', sortheader($MYREQUEST, 'C', 'Avg. Hits', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'<th>', sortheader($MYREQUEST, 'A', 'Avg. Size', "{$MY_SELF_WO_SORT}&OB=" . $MYREQUEST['OB']), '</th>',
		'</tr>';
