...
$urlArr = array(
	'建筑升级列表' => 'Base/BuildUpgList&page=1&id=' . $pageData['id'],
);
$info = $pageData['info'];

if (!empty($info) && is_array($info)) {
	$urlArr['修改升级建筑'] = 'Base/BuildUpgAdd&id=' . $info['build_id'] . '&level=' . $info['level'];
	$arrNeedBuild = !empty($info['need_build']) ? $info['need_build'] : array();
	$arrNeedTech = !empty($info['need_tech']) ? $info['need_tech'] : array();
	$arrEffect = !empty($info['effect']) ? json_decode($info['effect'], true) : array();
} else {
	$urlArr['新增升级建筑'] = 'Base/BuildUpgAdd&id=' . $pageData['id'];
...
<div class="top-bar">
	<h1><?php echo isset($info['id']) ? '修改建筑升级' : '新增建筑升级'; ?></h1>
...
<script type="text/javascript">
	function checkInput() {
		...
		if (document.getElementById('id').value == '') {
			window.alert('请填写建筑ID!');
			flag = false;
		}
		else if (document.getElementById('level').value == '' || isNaN(document.getElementById('level').value)) {
			window.alert('请填写建筑等级!');
			flag = false;
		}
...
<div class="table">
	<form action="?r=Base/DoBuildUpgAdd" method="post" onsubmit="javascript:return checkInput()">
		<table class="listing form" cellpadding="0" cellspacing="0">
			<tr>
				<td class="full" colspan="2">
					<font style="color:red"><?php echo isset($info['id']) ? '修改 ' : '新增 ';
						echo $pageData['buildBase'][$pageData['id']]['name']; ?></font> 升级数据
				</td>
			</tr>
				...
				<td>
					<select name='need_build_id' id='need_build_id'>
						...
						<?php foreach ($pageData['buildBase'] as $buildId => $buildInfo) { ?>
							<option
								value="<?php echo $buildId; ?>" <?php echo (isset($info['id']) && $buildId == key($arrNeedBuild)) ? 'selected' : ''; ?> >
					...
					<select name='need_build_level' id='need_build_level'>
						...
						<?php for ($i = 1; $i < 101; $i++) { ?>
							<option
								value="<?php echo $i; ?>" <?php echo (isset($info['id']) && $i == current($arrNeedBuild)) ? 'selected' : ''; ?> ><?php echo $i; ?></option>
						<?php } ?>
					</select>
				</td>
			</tr>
			...
			<tr>
				<td><strong>建筑效果</strong></td>
				<td>
					<select name='effect_code' id='effect_code'>
						<option value='0'>--请选择效果--</option>
						<?php foreach (T_Effect::$Build as $effect_code => $effect_desc) { ?>
							<option
