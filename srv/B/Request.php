 ...
 * @author huwei
 * @example
 *    $url = $protocol . '://wwww.xxxx.com/' . $script_name;
 * $cookie = array();
 * // 发起请求
 * $ret = B_Net_Request::call($url, $params, $cookie, $method, $protocol);
 * if (false === $ret['result']){print_r($ret);}
 */
class B_Request {
	/**
	 * 执行一个 HTTP 请求
	 ...
	 * @return array    结果数组
	 */
	static public function call($url, $params, $cookie = '', $method = 'post', $protocol = 'http') {
		...
		if (false === $ret || !empty($err)) {
			...
			trigger_error("errno:{$errno};msg:{$err};info:{$info}");
			return array(0, $err);
		}
